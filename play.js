...
(function() {
  var $, assert, choiceChars, compile, error, hashchange, mkElem, mkText, newGame, prepare, quote, restore, turn, _ref;
  ...
  _ref = require('./compiler'), quote = _ref.quote, prepare = _ref.prepare;
  ...
  exports.compile = compile = function(src) {
    ...
    (function() {
      ...
      _fn = function() {
        ...
        v.mungedSrc = v.src.replace(/\[\[([^\]]*)\]\]/g, function(outer, inner, index) {
          ...
          return "#\{_imbroglio.mkLink " + (quote(target)) + ", " + (quote(text)) + "}";
        });
    ...
    render = function(passage, result) {
      var linkCount, links, mkLink, moves, state, stateJSON, _imbroglio;
      if (result == null) {
        result = {};
      }
      ...
      result.passageElem = passage.prepared(_imbroglio = {
        mkLink: mkLink,
        state: state
      });
